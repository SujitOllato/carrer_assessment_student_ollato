...
const TestQuestion = () => {
  ...
  useEffect(() => {
    if (selectedQuestion?.path) {
      async function getImageUrl() {
        ...
        // setProfileImage(result?.url?.questionImg)
        const array = []
        array.push(result?.url?.questionImg)
        setBackImage(result?.url?.questionImg)
      }
      getImageUrl()
    }
  ...
  const nextQuestion = () => {
    ...
    if (selectedOptions.some((e) => +e.label === selectedCount)) {
      const hello = selectedOptions.find(
        (i) => i.label === selectedCount
      ).value
      // setselectedAns('')
      setisError(false)
            ...
            {(errorStatus === false || errorStatus === true) && (
              <div className="test-timer lesstime">
                ...
                <h4 className="lesstime">
                  <span>{moment.utc(counter * 1000).format('mm:ss')} Min </span>
                  Left
                </h4>
              </div>
            )}
                              ...
                              selectedQuestion?.options.map((opt, index) => {
                                return (
                                  ...
                                  >
                                    ...
                                    {opt?.is_image === true && opt?.path && (
                                      <div className="mb-3 test-question-image">
                                        <img
                                          src={images['option-' + (index + 1)]}
                                        />
                                      </div>
                                    )}
                                    <br />
