...
function EditMyProfile () {
    ...
    if (previousProps?.profileDataArray !== profileDataArray) {
      if (profileDataArray) {
        async function getImageUrl () {
          if (profileDataArray?.profile) {
            const data = [{
              path: profileDataArray?.profile,
              flag: 'profile'
            }]
  ...
  const onSubmit = async (data) => {
    const payload = {
      ...
      sCountryId: data?.country2?.id,
      sStateId: data?.state2?.id,
      sCityId: data?.district2?.id,
      sPinCode: +data?.pincode2,
      grade: data?.grade?.id,
      board: data?.board?.id,
      school_name: data?.school_name,
      sAddress1: data?.addressLine1,
      sAddress2: data?.addressLine2,
      countryId: data?.country2?.id,
      stateId: data?.state?.id,
      cityId: data?.district?.id,
      pinCode: +data?.pincode,
      mathDropped: mathsChecked,
      scienceDropped: scienceChecked,
      profile: profileDataArray?.profile,
    ...
    if (payload) {
      dispatch(editProfileAction(payload, token))
    }
  }
